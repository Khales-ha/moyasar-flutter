## Unreleased
- [Apple Pay] Fix a press being answered by the wrong widget. The hook and the payment result were exchanged on the plugin's shared channel, which holds a single handler, so with more than one button mounted the last one to register answered every press — running one order's pre-charge work and then presenting another's sheet — and its disposal left the earlier buttons vetoed for good. A hooked button is now given its own channel, named in its `creationParams`, and both its hook and its result travel on it. A button with no hook is unchanged, down to the bytes of its `creationParams`.
- [Apple Pay] Fix an approval returned after the widget was disposed. Removing the method handler does not cancel an invocation already in flight, so a hook completing `true` after the customer navigated away still presented the sheet over the new screen. The approval is now withheld unless the widget is still mounted, and unless the press came from the view the widget currently renders.
- [Apple Pay] Fix the charge being built from the live config once the sheet closed. The config approved at the press is now the one charged, so an order that changed while the sheet was up cannot move the charge onto it. It is scoped to that press: it is taken by the result the press produces, and a button pressed without a hook — which presents natively and never asks Dart — is charged what it is showing, so dropping the hook cannot leave a customer billed an amount they were never shown.
- [Apple Pay] Fix a button going silently dead when a sibling gained a hook. The plugin's shared channel holds a single handler; a widget moving to its own channel cleared that handler without checking it had installed it, so the sibling's sheet still presented, the customer still authorized, and nothing charged. Only the widget that installed a handler may clear it.
- [Apple Pay] Fix a payment sheet's result reaching the wrong button. A press with no hook skipped the serialization that makes the awaited hook safe, so a sheet presented while a hook was in flight could have its result delivered to the widget the hook was asked about. Every press is now serialized, and the native handler runs one payment session at a time.
- [Samsung Pay] Fix a charge for the wrong transaction. Only the hook was captured before the await: amount, currency, order number, merchant configuration, API key and card networks were all read from the live widget afterwards, so a rebuild during the hook turned an approval for one order into a sheet — and a charge — for another. Every payment input is now read once, at the tap.
- [Samsung Pay] Fix a `setState` after dispose when the native readiness check answers after the widget is gone.
